...
- The first Dietitian slice is implemented in code as deterministic review guidance for chart readiness, nutrition targets, meal-option completeness, and monitoring/handoff. It uses existing static workspace and chart-validation rules, accepts only an allowlisted workflow key, performs no AI call or application-record read, cannot be edited with record details, and remains default-off pending migrations 0186/0191 and its dedicated Development feature flag. It cannot calculate or recommend clinical targets, inspect a chart, submit, approve, publish, deliver, or contact anyone.
- The first Psychologist slice is implemented in code as deterministic workflow guidance for daily caseload orientation, consultation documentation, safety/concern escalation, and BluePrint/MDT handoff. It accepts only an allowlisted workflow key, performs no AI call or application-record read, cannot accept free-text record details, and remains default-off pending migrations 0186/0192 and its dedicated Development feature flag. It cannot read therapy notes, assess risk, diagnose, interpret assessments, recommend treatment, close a safety item, disclose content, refer, assign, or contact anyone.
- The first Doctor slice is implemented in code as deterministic workflow guidance for daily clinical orientation, consultation/EMR documentation, orders/results review orientation, and safety/MDT coordination. It accepts only an allowlisted workflow key, performs no AI call or application-record read, cannot accept free-text record details, and remains default-off pending migrations 0186/0193 and its dedicated Development feature flag. It cannot diagnose, interpret results, recommend treatment, prescribe, suggest a dose, place or approve an order, sign clinical content, assess risk, refer, message, or change any record.
- The first Medical Director slice is implemented in code as deterministic review guidance for queue orientation, evidence-completeness process, safety-escalation governance, and cross-discipline governance. It accepts only an allowlisted workflow key, performs no AI call or application-record read, cannot accept free-text record details, and remains default-off pending migrations 0186/0194 and its dedicated Development feature flag. It cannot inspect or rank a queue, decide completeness, interpret evidence, diagnose, recommend treatment, prescribe, assess risk, approve/reject/request changes, sign, publish, deliver, close, assign, contact, override a clinician, change a standard, or make a governance decision.
- The first Finance slice is implemented in code as deterministic process guidance for invoice/payment reconciliation, refund/void review preparation, expense/reimbursement evidence, and reporting/control review. It accepts only an allowlisted workflow key, performs no AI call or application-record read, cannot accept free-text record details, and remains default-off pending migrations 0186/0195 and its dedicated Development feature flag. It cannot calculate or assert amounts or finance states, inspect records, raise/edit invoices, record/capture payments, refund, void, credit, reimburse, approve, pay, post, reconcile, change subscriptions/prices/budgets/payroll, or contact anyone.
- The first HR slice is implemented in code as deterministic process guidance for onboarding/offboarding, attendance/leave, training/policy, and capacity/privacy review. It accepts only an allowlisted workflow key, performs no AI call or application-record read, cannot accept person or record details, and remains default-off pending migrations 0186/0196 and its dedicated Development feature flag. It cannot inspect personnel records, score or rank staff, infer protected traits, decide hiring/termination/discipline/performance/compensation/leave/access, update records, provision access, publish policy, assign work, or contact anyone.
- All 13 staff roles now resolve to an explicit bounded task definition. New task keys remain inert until an approved versioned manifest, server-side real-role policy and dedicated configuration exist; a future-looking environment flag alone cannot broaden a role.
...
1. Dietitian static review/navigation checklists implemented and guarded; record-aware completeness, calculation and review-packet tasks remain future scope;
...
3. Psychologist static workflow/safety-boundary checklists implemented and guarded; record-aware documentation and safety-escalation preparation remains future scope;
4. Doctor static workflow and clinical-boundary checklists implemented and guarded; record-aware pre-consult, documentation, order/result review, and decision support remain future scope; and
5. Medical Director static review-queue, evidence-process, safety-governance, and cross-discipline checklists implemented and guarded; record-aware evidence review, clinical decisions, approvals, and governance actions remain future scope.

No autonomous clinical action, approval, publication, or delivery.

### Phase 5 — Sensitive corporate functions

- Finance: static finance-process and transaction-boundary checklists implemented and guarded; record-aware deterministic reconciliation and explanatory drafts remain future scope.
- HR: static onboarding/offboarding, attendance/leave, training/policy, and capacity/privacy boundary checklists implemented and guarded; aggregate record-aware summaries, policy retrieval and drafts remain future scope.
...
3. Which additional task keys, beyond each role's bounded first pilot, enter the next evaluation phase and which are deferred?
...
Until those conditions are met, the correct application behavior is the current one: Health Coach and Super Admin remain guarded pilots; the implemented deterministic role pilots remain default-off behind their versioned migrations and role flags; any task without an approved manifest remains inert; voice remains disabled; and clients have no Cureocity Assistant access.
