...
const sql = readFileSync(resolve(process.cwd(), "supabase/0190_manager_assistant_pilot.sql"), "utf8");

describe("Manager Assistant pilot migration", () => {
  it("is forward-only, default-off and depends explicitly on migration 0186", () => {
    ...
    expect(sql).toContain("STAFF_COPILOT_MANAGER_ENABLED=true");
  ...
  it("enforces the exact real role, task version and workflow allowlist", () => {
    expect(sql).toContain("v_role <> 'Manager'");
    expect(sql).toContain("p_task_version is distinct from 'manager.operations_checklist.v1'");
    expect(sql).toContain("p_policy_version is distinct from '2026-08-17.1'");
    for (const workflow of ["coverage_coordination", "coach_quality_review", "onboarding_handover", "service_operations_review"]) expect(sql).toContain(`'${workflow}'`);
  });
  ...
  it("grants only authenticated execution and stores deterministic public/operational metadata", () => {
    ...
    expect(sql).toContain("revoke all on function create_manager_assistant_draft");
    expect(sql).toContain("grant execute on function create_manager_assistant_draft");
