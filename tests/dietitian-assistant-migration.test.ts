...
const sql = readFileSync(resolve(process.cwd(), "supabase/0191_dietitian_assistant_pilot.sql"), "utf8");

describe("Dietitian Assistant pilot migration", () => {
  it("is forward-only, default-off and depends explicitly on migration 0186", () => {
    ...
    expect(sql).toContain("STAFF_COPILOT_DIETITIAN_ENABLED=true");
  ...
  it("enforces the exact real role, task version and workflow allowlist", () => {
    expect(sql).toContain("v_role <> 'Dietitian'");
    expect(sql).toContain("p_task_version is distinct from 'dietitian.review_checklist.v1'");
    expect(sql).toContain("p_policy_version is distinct from '2026-08-17.1'");
    for (const workflow of ["chart_review_readiness", "nutrition_targets", "meal_option_completeness", "monitoring_and_handoff"]) expect(sql).toContain(`'${workflow}'`);
  });

  it("constructs persisted text in the database and audits each transition", () => {
  ...
  it("grants only authenticated execution and stores deterministic metadata", () => {
    expect(sql).toContain("'deterministic', array['Public application metadata', 'Internal operational']");
    expect(sql).toContain("revoke all on function create_dietitian_assistant_draft");
    expect(sql).toContain("grant execute on function create_dietitian_assistant_draft");
