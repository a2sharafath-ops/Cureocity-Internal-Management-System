...
export default function CoachProgrammeLifecyclePanel({ clientId, lifecycle, events, canManage, supervisorOverride, today }: {
  ...
  const [target, setTarget] = useState<CoachProgrammeStatus | "">("");
  ...
  return <section id="programme-lifecycle" style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", boxShadow: "var(--shadow)", padding: "16px 18px", marginBottom: 16 }}>
    ...
    {canManage && <details style={{ marginTop: 12, border: "1px solid var(--border)", borderRadius: 9, padding: "9px 11px" }}>
      <summary style={{ cursor: "pointer", fontSize: 12.5, fontWeight: 700 }}>{current === "Active" ? "Change programme status" : "Record next lifecycle transition"}</summary>
      <form action={action} style={{ display: "grid", gap: 9, marginTop: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <label style={label}>New status<select name="to_status" required value={target} onChange={(event) => setTarget(event.target.value as CoachProgrammeStatus)} style={field}><option value="" disabled>Choose status</option>{options.map((status) => <option key={status}>{status}</option>)}</select></label>
        ...
        <label style={label}>Reason<textarea name="reason" required minLength={12} maxLength={1000} rows={3} style={{ ...field, resize: "vertical" }} placeholder="Record the client context and agreed decision." /></label>
        ...
        <div style={{ color: "var(--muted)", fontSize: 10.5 }}>Transitions are permanent events. Corrections are recorded as another allowed transition.</div>
        <button type="submit" style={{ ...button, justifySelf: "start" }}>Record transition</button>
      </form>
    </details>}

    {events.length > 0 && <details style={{ marginTop: 10 }}><summary style={{ cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Lifecycle history ({events.length})</summary><div style={{ display: "grid", gap: 6, marginTop: 7 }}>{events.map((event) => <div key={event.id} style={{ borderTop: "1px solid var(--border)", paddingTop: 6, fontSize: 11.5 }}><b>{event.effective_date} · {event.from_status} → {event.to_status}</b><div>{event.reason}</div>{event.next_contact_date && <div style={{ color: "var(--muted)" }}>Next contact {event.next_contact_date} · {event.next_contact_plan}</div>}<div style={{ color: "var(--muted)", fontSize: 10.5 }}>{event.actor_name} ({event.actor_role})</div></div>)}</div></details>}
  </section>;
}
