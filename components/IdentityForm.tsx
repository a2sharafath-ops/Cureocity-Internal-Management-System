...
export default function IdentityForm({ clients }: { clients: { id: string; name: string; abha_id: string | null; uhid: string | null }[] }) {
  ...
  if (!open) return <button type="button" onClick={() => setOpen(true)} style={{ background: "#fff", color: "var(--teal-dark)", border: "1px solid var(--teal)", borderRadius: 10, padding: "8px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+ Link identity</button>;
  return (
