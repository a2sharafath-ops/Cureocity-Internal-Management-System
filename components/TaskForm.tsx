...
export default function TaskForm({ staff, clients }: { staff: { id: string; name: string }[]; clients: { id: string; name: string }[] }) {
  ...
  if (!open) return <button type="button" onClick={() => setOpen(true)} style={{ background: "var(--teal)", color: "#fff", border: "none", borderRadius: 10, padding: "9px 15px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+ New task</button>;
  return (
