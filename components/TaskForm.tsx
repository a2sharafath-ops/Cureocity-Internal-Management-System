...
export default function TaskForm({ staff, clients, projects = [] }: { staff: { id: string; name: string }[]; clients: { id: string; name: string }[]; projects?: { id: string; name: string }[] }) {
  ...
  return (
    <form action={addTask} onSubmit={() => setTimeout(() => setOpen(false), 50)} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", boxShadow: "var(--shadow)", padding: 16, marginBottom: 16, display: "grid", gap: 10 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.25fr 1.25fr 1.25fr 1fr 1fr 1fr auto", gap: 10, alignItems: "end" }}>
        <div style={{ display: "grid", gap: 3 }}><label style={lbl}>Assignees <span style={{ fontWeight: 400 }}>(choose one or more)</span></label><select style={{ ...input, height: 76, padding: 6 }} name="assignee_ids" multiple aria-label="Task assignees">{staff.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
        {projects.length > 0 && <div style={{ display: "grid", gap: 3 }}><label style={lbl}>Project</label><select style={input} name="project_id" defaultValue=""><option value="">Operations inbox</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></div>}
