...
import {
  acceptFitnessTrainerOperationalDraft,
  discardFitnessTrainerOperationalDraft,
...
export type FitnessTrainerAssistantHistory = {
  id: string;
  title: string;
  draft_text: string;
...
function DraftReview({ draft }: { draft: NonNullable<FitnessTrainerAssistantState["draft"]> }) {
  ...
  const run = (operation: "accept" | "discard") => startTransition(async () => {
    ...
    const result = operation === "accept"
      ? await acceptFitnessTrainerOperationalDraft(draft.id)
      : await discardFitnessTrainerOperationalDraft(draft.id);
    if (result.error) setError(result.error);
    else setMessage(result.ok ?? "Done");
  });

  return (
    <section style={{ ...box, padding: 16, display: "grid", gap: 10 }} aria-label="Review Fitness Trainer workspace checklist">
      ...
      <textarea
        aria-label="Fitness Trainer workspace checklist"
        rows={11}
        value={draft.text}
        readOnly
      ...
      {draft.evidence.length > 0 && (
        <details>
          <summary style={{ cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Static workspace evidence</summary>
      ...
      <div style={{ color: "var(--muted)", fontSize: 11 }}>This static checklist cannot be edited with record details. Accepting stores the generated text only; it does not inspect a client, prescribe exercise, schedule or complete a session, publish a plan, or change Cureocity.</div>
    </section>
  );
}

export default function FitnessTrainerAssistant({
  history,
  enabled,
  historyError,
}: {
  history: FitnessTrainerAssistantHistory[];
  enabled: boolean;
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ ...box, padding: 16, background: "#eff6ff", color: "#1e3a8a", fontSize: 12.5 }}>
        <b>Fitness Trainer workspace checklist pilot.</b> Choose a workflow to receive static navigation guidance. It calls no AI provider and reads no client, clinical, assessment, workout, session, finance, HR, staff or message records.
      </div>

      {!enabled && (
        <div style={{ ...box, padding: 16, color: "var(--amber-text)", background: "var(--amber-bg)", fontSize: 12.5 }}>
          <b>This pilot is not active.</b> Migrations 0186 and 0188 must be applied in the intended Development environment and the dedicated Fitness Trainer feature flag must be enabled. The default deployed state remains off.
        </div>
      )}
      ...
      <form action={action} style={{ ...box, padding: 16, display: "grid", gap: 10 }}>
        ...
        <div style={{ color: "var(--muted)", fontSize: 11.5 }}>
          Do not enter client or clinical details. This selector sends only the workflow key and uses versioned static workspace metadata.
        </div>
      ...
      <section style={{ ...box, overflow: "hidden" }}>
        <div style={{ padding: "12px 15px" }}>
          <b style={{ fontSize: 13.5 }}>Recent Fitness Trainer checklists</b>
          <div style={{ color: "var(--muted)", fontSize: 11.5, marginTop: 2 }}>Generated workspace evidence is retained; accepted text remains separate and performs no action.</div>
        </div>
        {historyError ? (
          ...
          <details key={item.id} style={{ borderTop: "1px solid var(--border)", padding: "10px 15px" }}>
            ...
            <div style={{ color: "var(--muted)", fontSize: 10.5, marginTop: 6 }}>Static Fitness Trainer workspace draft · no AI call · no record read · no action executed</div>
          </details>
        )) : (
          <div style={{ borderTop: "1px solid var(--border)", padding: 15, color: "var(--muted)", fontSize: 12.5 }}>No Fitness Trainer checklists yet.</div>
        )}
      </section>
    </div>
  );
