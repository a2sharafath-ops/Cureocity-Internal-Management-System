...
export default function FollowupActions({ id }: { id: string }) {
  ...
  if (logging) {
    return (
      <form action={completeFollowup} style={{ display: "flex", gap: 6, alignItems: "center", justifyContent: "flex-end" }} onSubmit={() => setLogging(false)}>
        ...
        <button type="submit" style={{ ...btn, borderColor: "var(--teal)", color: "var(--teal-dark)" }}>Done</button>
  ...
  return (
    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
      <button type="button" onClick={() => setLogging(true)} style={{ ...btn, borderColor: "var(--teal)", color: "var(--teal-dark)" }}>✓ Mark done</button>
