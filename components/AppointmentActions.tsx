...
const rescheduleAppointmentForm = async (formData: FormData) => {
...
export default function AppointmentActions({ id, status, date, hour, canEdit = true }: { id: string; status: string; date?: string; hour?: number; canEdit?: boolean }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>
      ...
      {resch && (
        <form action={rescheduleAppointmentForm} onSubmit={() => { setResch(false); setOpen(false); }} style={{ display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap" }}>
