  ...
  return (
    <form action={createTemplate} onSubmit={() => setTimeout(() => setOpen(false), 50)} style={{ ...panel, display: "grid", gap: 10 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 2fr", gap: 10 }}>
        <Field label="Template name"><input style={inputControl} name="name" required /></Field>
        <Field label="Category"><select style={inputControl} name="category" defaultValue="General"><option>General</option><option>Onboarding</option><option>Retention</option><option>Billing</option><option>Promo</option></select></Field>
        <Field label="Subject"><input style={inputControl} name="subject" required /></Field>
      </div>
...
export function SendCampaign({ id, configured }: { id: string; configured: boolean }) {
  return (
    <form action={sendCampaignNow}>
      ...
      <button type="submit" style={{ border: "1px solid var(--brand-fill)", background: configured ? "var(--brand-fill)" : "#fff", color: configured ? "#fff" : "var(--brand-text)", borderRadius: 8, padding: "4px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
