...
export default function TargetForm({ month, revenue, newClients, renewals }: { month: string; revenue: number; newClients: number; renewals: number }) {
  ...
  if (!open) return <button type="button" onClick={() => setOpen(true)} style={{ background: "var(--teal)", color: "#fff", border: "none", borderRadius: 10, padding: "9px 15px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>⚙️ Set targets</button>;
  return (
