...
import {
  generateAdministratorGovernanceDraft,
  type AdministratorAssistantState,
} from "@/lib/administrator-assistant-actions";
import { ADMINISTRATOR_WORKFLOWS } from "@/lib/administrator-assistant";
import { generateManagerOperationsDraft, type ManagerAssistantState } from "@/lib/manager-assistant-actions";
import { MANAGER_WORKFLOWS } from "@/lib/manager-assistant";
import { generateDietitianReviewDraft, type DietitianAssistantState } from "@/lib/dietitian-assistant-actions";
...
function AdministratorQuickDraft() {
  const [state, action, pending] = useActionState<AdministratorAssistantState, FormData>(
    generateAdministratorGovernanceDraft,
    {},
  );
    ...
    <div style={{ display: "grid", gap: 10 }}>
      <form action={action} style={{ display: "grid", gap: 9 }}>
        <label style={{ display: "grid", gap: 4, fontSize: 12, fontWeight: 700 }}>
          ...
          <select name="workflow_key" defaultValue="access_governance" style={input}>
            {ADMINISTRATOR_WORKFLOWS.map((workflow) => (
              <option key={workflow.key} value={workflow.key}>{workflow.label}</option>
            ))}
          </select>
        </label>
        <div style={{ color: "var(--muted)", fontSize: 11.5, lineHeight: 1.45 }}>
          No record details are requested. The Assistant uses only the selected workflow key and versioned static route metadata.
        </div>
        {state.error && (
          <div role="alert" style={{ borderRadius: 9, padding: "9px 10px", background: "var(--red-bg)", color: "var(--red-text)", fontSize: 12 }}>
            {state.error}
        ...
        <button type="submit" disabled={pending} style={{ border: 0, borderRadius: 10, padding: "10px 13px", background: "var(--ink)", color: "#fff", fontWeight: 750, cursor: pending ? "default" : "pointer", opacity: pending ? 0.65 : 1 }}>
          {pending ? "Preparing checklist…" : "Prepare governance checklist"}
        </button>
      </form>
      {state.draft && (
        <section aria-label="Generated Administrator governance checklist" style={{ border: "1px solid var(--border)", borderRadius: 11, padding: 12, display: "grid", gap: 8, background: "var(--neutral-bg)" }}>
          <div style={{ color: "#5b21b6", fontSize: 10.5, fontWeight: 800 }}>DETERMINISTIC DRAFT · STATIC ROUTES · NO AI CALL · NO RECORD READ · NO ACTION</div>
          <b style={{ fontSize: 13.5 }}>{state.draft.title}</b>
          <div style={{ whiteSpace: "pre-wrap", fontSize: 12, lineHeight: 1.5 }}>{state.draft.text}</div>
          <div style={{ color: "var(--amber-text)", fontSize: 11.5 }}><b>Boundary:</b> {state.draft.caution}</div>
          <div style={{ color: "var(--muted)", fontSize: 11 }}>Continue in the full workspace to review, accept, discard, or inspect route evidence. Record details cannot be added.</div>
...
function ManagerQuickDraft() {
  const [state, action, pending] = useActionState<ManagerAssistantState, FormData>(generateManagerOperationsDraft, {});
  return (
    <div style={{ display: "grid", gap: 10 }}>
      <form action={action} style={{ display: "grid", gap: 9 }}>
        <label style={{ display: "grid", gap: 4, fontSize: 12, fontWeight: 700 }}>
          Manager workflow
          <select name="workflow_key" defaultValue="coverage_coordination" style={input}>
            {MANAGER_WORKFLOWS.map((workflow) => <option key={workflow.key} value={workflow.key}>{workflow.label}</option>)}
          </select>
        </label>
        <div style={{ color: "var(--muted)", fontSize: 11.5, lineHeight: 1.45 }}>No record details are requested. The Assistant uses only the selected workflow key and versioned static route metadata.</div>
        ...
        <button type="submit" disabled={pending} style={{ border: 0, borderRadius: 10, padding: "10px 13px", background: "var(--ink)", color: "#fff", fontWeight: 750, cursor: pending ? "default" : "pointer", opacity: pending ? 0.65 : 1 }}>
          {pending ? "Preparing checklist…" : "Prepare operations checklist"}
        </button>
      </form>
      {state.draft && (
        <section aria-label="Generated Manager operations checklist" style={{ border: "1px solid var(--border)", borderRadius: 11, padding: 12, display: "grid", gap: 8, background: "var(--neutral-bg)" }}>
          <div style={{ color: "#5b21b6", fontSize: 10.5, fontWeight: 800 }}>DETERMINISTIC DRAFT · STATIC ROUTES · NO AI CALL · NO RECORD READ · NO ACTION</div>
          <b style={{ fontSize: 13.5 }}>{state.draft.title}</b>
          <div style={{ whiteSpace: "pre-wrap", fontSize: 12, lineHeight: 1.5 }}>{state.draft.text}</div>
          <div style={{ color: "var(--amber-text)", fontSize: 11.5 }}><b>Boundary:</b> {state.draft.caution}</div>
  ...
  return (
    <div style={{ display: "grid", gap: 10 }}>
      <form action={action} style={{ display: "grid", gap: 9 }}>
        ...
        <div style={{ color: "var(--muted)", fontSize: 11.5, lineHeight: 1.45 }}>No record details are requested. The Assistant uses only the selected workflow key and versioned static route and review-rule metadata.</div>
        {state.error && <div role="alert" style={{ borderRadius: 9, padding: "9px 10px", background: "var(--red-bg)", color: "var(--red-text)", fontSize: 12 }}>{state.error}</div>}
        <button type="submit" disabled={pending} style={{ border: 0, borderRadius: 10, padding: "10px 13px", background: "var(--ink)", color: "#fff", fontWeight: 750, cursor: pending ? "default" : "pointer", opacity: pending ? 0.65 : 1 }}>
      ...
      {state.draft && (
        <section aria-label="Generated Dietitian review checklist" style={{ border: "1px solid var(--border)", borderRadius: 11, padding: 12, display: "grid", gap: 8, background: "var(--neutral-bg)" }}>
          ...
          <div style={{ whiteSpace: "pre-wrap", fontSize: 12, lineHeight: 1.5 }}>{state.draft.text}</div>
          <div style={{ color: "var(--amber-text)", fontSize: 11.5 }}><b>Boundary:</b> {state.draft.caution}</div>
          <div style={{ color: "var(--muted)", fontSize: 11 }}>Continue in the full workspace to review, accept, discard, or inspect static rule evidence. Record details cannot be added.</div>
        </section>
      )}
    </div>
  );
...
function PsychologistQuickDraft() {
  const [state, action, pending] = useActionState<PsychologistAssistantState, FormData>(generatePsychologistReviewDraft, {});
  return (
    <div style={{ display: "grid", gap: 10 }}>
      <form action={action} style={{ display: "grid", gap: 9 }}>
        <label style={{ display: "grid", gap: 4, fontSize: 12, fontWeight: 700 }}>
          Psychologist workflow
          <select name="workflow_key" defaultValue="daily_caseload_orientation" style={input}>
            {PSYCHOLOGIST_WORKFLOWS.map((workflow) => <option key={workflow.key} value={workflow.key}>{workflow.label}</option>)}
          </select>
        </label>
        <div style={{ color: "var(--muted)", fontSize: 11.5, lineHeight: 1.45 }}>No record details are requested. The Assistant uses only the selected workflow key and versioned static route and safety-boundary metadata.</div>
        {state.error && <div role="alert" style={{ borderRadius: 9, padding: "9px 10px", background: "var(--red-bg)", color: "var(--red-text)", fontSize: 12 }}>{state.error}</div>}
      ...
      {state.draft && (
        <section aria-label="Generated Psychologist workflow checklist" style={{ border: "1px solid var(--border)", borderRadius: 11, padding: 12, display: "grid", gap: 8, background: "var(--neutral-bg)" }}>
          <div style={{ color: "#5b21b6", fontSize: 10.5, fontWeight: 800 }}>DETERMINISTIC DRAFT · STATIC ROUTES · NO AI CALL · NO RECORD READ · NO ACTION</div>
          <b style={{ fontSize: 13.5 }}>{state.draft.title}</b>
          <div style={{ whiteSpace: "pre-wrap", fontSize: 12, lineHeight: 1.5 }}>{state.draft.text}</div>
          <div style={{ color: "var(--amber-text)", fontSize: 11.5 }}><b>Boundary:</b> {state.draft.caution}</div>
          <div style={{ color: "var(--muted)", fontSize: 11 }}>Continue in the full workspace to review, accept, discard, or inspect static route evidence. Record details cannot be added.</div>
        </section>
      )}
    </div>
  );
      ...
      {open && (
        <div style={{ position: "fixed", inset: 0, zIndex: 80 }}>
          ...
          >
            ...
            ) : surface.quickPromptKind === "administrator_checklist" ? (
              <AdministratorQuickDraft />
            ) : surface.quickPromptKind === "manager_checklist" ? (
              <ManagerQuickDraft />
            ) : surface.quickPromptKind === "dietitian_checklist" ? (
              <DietitianQuickDraft />
