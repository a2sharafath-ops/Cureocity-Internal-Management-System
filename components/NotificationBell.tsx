      ...
      {open && (
        <>
          ...
          <div style={{ position: "absolute", right: 0, top: 40, width: 320, maxHeight: 420, overflow: "auto", background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "0 12px 30px rgba(0,0,0,0.14)", zIndex: 41 }}>
            ...
            {items.map((n) => {
              const body = (
                <div style={{ display: "flex", gap: 10, padding: "10px 14px", borderBottom: "1px solid var(--border)", background: n.read ? "transparent" : "rgba(13,148,136,0.06)" }}>
                  ...
                  {!n.read && <span style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--brand-fill)", marginTop: 5 }} />}
                </div>
              );
              return (
            ...
            <Link href="/alerts" onClick={() => setOpen(false)} style={{ display: "block", textAlign: "center", padding: "11px 14px", borderTop: "1px solid var(--border)", color: "var(--brand-text)", fontSize: 13, fontWeight: 600, textDecoration: "none", position: "sticky", bottom: 0, background: "var(--card)" }}>
