...
export default function AssignWorkout({ templateId, clients }: { templateId: string; clients: { id: string; name: string }[] }) {
  ...
  // Deep-linked from a "workout plan not created" reminder: ?client=<id>
  // pre-selects that client so the trainer just picks a template and assigns.
  ...
  return (
    <form action={assignWorkout} onSubmit={() => setTimeout(() => setOpen(false), 50)} style={{ display: "flex", gap: 6, alignItems: "center" }}>
      ...
      <select name="client_id" required defaultValue={focusClient} style={{ border: "1px solid var(--border)", borderRadius: 8, padding: "4px 8px", fontSize: 12, background: "#fff" }}>
