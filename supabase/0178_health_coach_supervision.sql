-- ============================================================================
-- Cureocity — Health Coach supervision authority. Run after 0177.
--
-- The Medical Director is the Health Coach's clinical supervisor. Super Admin
-- retains emergency platform authority. Administrator and Manager keep their
-- normal visibility, but can no longer mutate or override Health Coach-owned
-- client records.
-- ============================================================================

begin;

create or replace function is_health_coach_supervisor()
returns boolean
language sql
security definer
stable
...
as $$
  select coalesce(my_role() in ('Super Admin', 'Medical Director'), false)
$$;

comment on function is_health_coach_supervisor() is
  'True only for the Medical Director or Super Admin.';
...
as $$
  select coalesce(
    is_health_coach_supervisor()
    or is_assigned_health_coach(target_client_id),
    false
  )
$$;

comment on function can_write_health_coach_client(uuid) is
  'Assigned Health Coach, Medical Director or Super Admin. Supervisor reasons are enforced and audited by server actions.';
...
revoke all on function is_health_coach_supervisor() from public;
grant execute on function is_health_coach_supervisor() to authenticated;
