...
export default async function TargetsPage() {
  ...
  const [{ data: targetRow }, { data: paidInv }, clientsC, { data: renewInv }] = await Promise.all([
  ...
  const goal = (title: string, val: number, tgt: number, fmt: (n: number) => string, note: string) => {
  ...
  return (
    <div style={{ maxWidth: 940 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {goal("💰 Revenue Target", revenue, target.revenue_target, money, `${((paidInv ?? []) as unknown[]).length} payments collected this month`)}
