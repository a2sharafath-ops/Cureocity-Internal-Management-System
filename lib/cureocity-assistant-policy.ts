...
const psychologistReviewManifest = manifest({
  taskVersion: "psychologist.workflow_checklist.v1",
  role: "Psychologist",
  key: PSYCHOLOGIST_REVIEW_TASK_KEY,
  label: "Prepare a Psychologist workflow checklist",
  owner: "Cureocity Psychologist domain owner and Medical Director",
  implementation: "implemented",
  executionMode: "deterministic",
  featureFlag: "STAFF_COPILOT_PSYCHOLOGIST_ENABLED",
  requiresExternalAi: false,
  actionTier: 1,
  reviewerRoles: ["Psychologist"],
  dataContract: {
    sources: ["versioned Psychologist workspace and safety-boundary metadata", "authenticated real role", "existing route and tab permission map"],
    allowedFields: ["workflow label", "workspace tab label", "route path", "static purpose", "static process boundary"],
    classifications: ["Public application metadata", "Internal operational"],
    forbiddenSources: ["client records", "psychological records", "clinical records", "consultation content", "assessment content", "therapy notes", "appointment records", "concern records", "safety records", "referrals", "finance records", "HR records", "staff records", "messages", "credentials"],
  },
  prohibitedActions: [
    ...NO_ACTIONS,
    "diagnose, interpret an assessment, recommend treatment, provide psychotherapy or crisis advice, or disclose therapy content",
    "book, change, submit, escalate, close, refer, assign, contact, or claim that a clinical or safety item is complete, safe, urgent, or resolved",
  ],
});

const doctorReviewManifest = manifest({
  taskVersion: "doctor.workflow_checklist.v1",
  role: "Doctor",
  key: DOCTOR_REVIEW_TASK_KEY,
  label: "Prepare a Doctor workflow checklist",
  owner: "Cureocity Doctor domain owner and Medical Director",
  implementation: "implemented",
  executionMode: "deterministic",
  featureFlag: "STAFF_COPILOT_DOCTOR_ENABLED",
  requiresExternalAi: false,
  actionTier: 1,
  reviewerRoles: ["Doctor"],
  dataContract: {
    sources: ["versioned Doctor workspace and clinical-boundary metadata", "authenticated real role", "existing route and tab permission map"],
    allowedFields: ["workflow label", "route label", "route path", "static purpose", "static clinical boundary"],
    classifications: ["Public application metadata", "Internal operational"],
    forbiddenSources: ["client records", "medical records", "clinical records", "consultation content", "EMR content", "results", "orders", "prescriptions", "notes", "appointment records", "concern records", "safety records", "referrals", "finance records", "HR records", "staff records", "messages", "credentials"],
  },
  prohibitedActions: [
    ...NO_ACTIONS,
    "diagnose, interpret a result, recommend treatment, prescribe, select a test, place an order, suggest a dose, or provide crisis advice",
    "sign, approve, submit, publish, deliver, close, refer, assign, contact, or claim that a clinical or safety item is complete, safe, indicated, urgent, or resolved",
  ],
});
